...
const TermsConditionForm = () => {
  ...
  const fetchTermsConditions = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/pages?slug=terms-conditions`, {
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.data && data.data.length > 0) {
          ...
          console.log('No Terms & Conditions found, using defaults')
        }
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Saving terms & conditions with slug:', formData.slug)
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to save terms & conditions')
      }

      const data = await response.json()
      setPageData(data.data)
      setSuccess(pageData ? 'Terms & Conditions updated successfully!' : 'Terms & Conditions created successfully!')
  ...
  const handlePublish = async () => {
    ...
    setTimeout(() => {
      document.getElementById('terms-form').requestSubmit()
    }, 100)
  }
    ...
    return (
      <Card>
        <CardBody className="text-center py-5">
          ...
          <p className="mt-2 text-muted">Loading terms & conditions...</p>
        </CardBody>
      </Card>
    )
  }
          ...
          <CardBody>
            <div className="d-flex justify-content-between">
              ...
              <div className="d-flex gap-2">
                <Button variant="outline-secondary" onClick={fetchTermsConditions} disabled={saving}>
                ...
                <Button variant="primary" type="submit" disabled={saving}>
                  {saving ? (
                    ...
                    <>
                      ...
                      {pageData ? 'Update' : 'Create'} Terms & Conditions
                    </>
                  )}
                </Button>
              </div>
